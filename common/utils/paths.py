...
Usage:
    from common.utils.paths import setup_paths, get_project_root, get_data_dir
...
def get_project_root() -> Path:
    ...
    for parent in [current] + list(current.parents):
        ...
        has_common = (parent / 'common').is_dir()
        has_config = (parent / 'config').is_dir()
        has_git = (parent / '.git').exists()

        if has_common and (has_config or has_git):
            _PROJECT_ROOT = parent
            return parent

    # Fallback: assume common/utils/paths.py structure
...
def setup_paths() -> None:
    ...
    from common/ without being in the project root.
    """
    root = get_project_root()
    root_str = str(root)
...
def get_src_dir() -> Path:
    """Get the common/ directory path (legacy alias for get_common_dir)."""
...
# This allows simple usage: from common.utils.paths import *
setup_paths()
...
if __name__ == '__main__':
    ...
    print(f"Common Dir:   {get_common_dir()}")
