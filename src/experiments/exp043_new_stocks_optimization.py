...
MOTIVATION:
EXP-042 discovered 8 new Tier A stocks with >70% win rates
- All currently using baseline parameters (z=1.5, rsi=35, vol=1.3)
- EXP-037/038 showed +5-28pp improvements from optimization
- Optimizing these stocks completes portfolio optimization (25/31 stocks)

PROBLEM:
New stocks underoptimized:
- QCOM: 80% baseline - could hit 85-90% optimized
- JPM: 75% baseline - could hit 80-85% optimized
- AMAT: 80% baseline - could hit 85-90% optimized
- NOW: 73% baseline - could hit 80-85% optimized
- Plus JNJ, PFE, WMT, ADI

HYPOTHESIS:
Grid search optimization will improve these stocks by +5-15pp:
- Each stock has unique characteristics requiring tuned parameters
- Expected average improvement: +10pp (based on EXP-037/038 patterns)
- Portfolio win rate: ~75% → ~78%+

METHODOLOGY:
1. Grid search optimization (same as EXP-037):
...
5. Update to v13.4-OPTIMIZED

EXPECTED OUTCOME:
- 8 stocks optimized with stock-specific parameters
...
def run_exp043_new_stocks_optimization():
    """
    Optimize parameters for 8 newly added Tier A stocks.
    """
    print("=" * 70)
    print("EXP-043: NEW STOCKS PARAMETER OPTIMIZATION")
    print("=" * 70)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()

    print("Objective: Optimize parameters for 8 new Tier A stocks from EXP-042")
    print("Current: All stocks using baseline parameters (z=1.5, rsi=35, vol=1.3)")
    print("Expected: +5-15pp improvement per stock (based on EXP-037/038 patterns)")
    print()

    # New stocks from EXP-042
    new_stocks = ['QCOM', 'JPM', 'JNJ', 'PFE', 'WMT', 'AMAT', 'ADI', 'NOW']

    print(f"Optimizing {len(new_stocks)} stocks: {', '.join(new_stocks)}")
    ...
    if optimization_results:
        ...
        if deploy_count > 0:
            ...
            print("[NO DEPLOYMENT] All stocks performing well with baseline parameters")
    ...
    results_file = os.path.join(results_dir, 'exp043_new_stocks_optimization.json')
    ...
    try:
        ...
        if notifier.is_enabled():
            ...
            notifier.send_experiment_report('EXP-043', exp_results)
        else:
...
if __name__ == '__main__':
    """Run EXP-043 new stocks optimization."""

    print("\n[NEW STOCKS OPTIMIZATION] Optimizing 8 newly added Tier A stocks")
    print("This will take 20-30 minutes...")
    print()

    results = run_exp043_new_stocks_optimization()

    print("\n" + "=" * 70)
    print("NEXT STEPS")
    print("=" * 70)
    print("1. Deploy optimized parameters to mean_reversion_params.py")
    print("2. Update to v13.4-OPTIMIZED")
    print("3. Portfolio optimization complete: 25/31 stocks (81%)")
    print()
    print("NEW STOCKS OPTIMIZATION COMPLETE")
